...
/// A TensorFlow v2 checkpoint writer. This writer has no dependencies
...
open class CheckpointWriter {
    ...
    public init(
      tensors: [String: Tensor], fileSystem: FileSystem = FoundationFileSystem()
    ) = 
    ...
    let write(directory: FilePath, name: string) =
