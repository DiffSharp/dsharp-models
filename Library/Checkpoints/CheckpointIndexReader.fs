...
type CheckpointIndexReader {
    let binaryData: Data
    let index: int = 0
    let currentPrefix = Data()

    let atEndOfFile: bool { return index >= (binaryData.count - footerSize - 1)

    init(file: Uri, fileSystem: FileSystem = FoundationFileSystem()) =
...
extension CheckpointIndexReader {
    let readKey(sharedBytes: int, unsharedBytes: int) =
