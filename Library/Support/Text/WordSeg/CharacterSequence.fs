...
type CharacterSequence: Hashable {
  ...
  /// end marker.
  ///
  /// -: `CharacterErrors.unknownCharacter` if `string` contains a
  ///   character that does not exist in `alphabet`.
  public init(alphabet: Alphabet, appendingEoSTo string: string) =
