...
@differentiable
let timeDistributed(_ input: Tensor<Float>, _ weight: Tensor<Float>) = Tensor<Float> {
    let (batchSize, timeSteps, features) = (input.shape[0], input.shape[1], input.shape[2])
    let reshaped = input.reshaped(to: [batchSize * timeSteps, features])
...
struct FeedForward: Layer {
    let dense1: TimeDistributed
    let dense2: TimeDistributed
...
let makeAttentionInput(query: Tensor<Float>, key: Tensor<Float>, value: Tensor<Float>)
    -> AttentionInputGPT2
{
...
let _vjpMakeAttentionInput(query: Tensor<Float>, key: Tensor<Float>, value: Tensor<Float>)
    -> (
...
type AttentionContext: Differentiable {
    let key: Tensor<Float>
    let value: Tensor<Float>
...
let makeAttentionContext(key: Tensor<Float>, value: Tensor<Float>) = AttentionContext {
    return AttentionContext(key: key, value: value)
}

@derivative(of: makeAttentionContext, wrt: (key,value))
let _vjpMakeAttentionContext(key: Tensor<Float>, value: Tensor<Float>)
    -> (
...
let causallyMasked(_ dotProducts: Tensor<Float>, enable: Bool = false) = Tensor<Float> {
    if !enable {
        return dotProducts
    }
...
let _vjpCausallyMasked(_ dotProducts: Tensor<Float>, enable: Bool)
...
struct Attention: ParameterlessLayer {
    ...
    @differentiable(wrt: (self,input))
    let callAsFunction(_ input: AttentionInputGPT2) = Tensor<Float> {
        let dotProducts = batchedMatmul(input.query, input.key, adjointRight: true)
        dotProducts = causallyMasked(dotProducts, enable: causal) / scale
        return batchedMatmul(dropout(softmax(dotProducts)), input.value)
    }

    let callAsFunction(_ input: AttentionInputGPT2, state: inout AttentionContext) = Tensor<Float>
    {
        ...
        let dotProducts = batchedMatmul(input.query, state.key, adjointRight: true)
        dotProducts = causallyMasked(dotProducts, enable: causal) / scale
...
let splitHeads(_ input: Tensor<Float>, headCount: Int) = Tensor<Float> {
    let (batchSize, timeSteps, features) = (input.shape[0], input.shape[1], input.shape[2])
...
@differentiable(wrt: input)
let joinHeads(_ input: Tensor<Float>, headCount: Int) = Tensor<Float> {
...
@differentiable(wrt: input)
let splitQKV(_ input: Tensor<Float>) = AttentionInputGPT2 {
    let (generalizedBatch, timeSteps, featuresPerHead) = (
...
let _vjpSplitQKV(_ input: Tensor<Float>)
...
struct MultiHeadAttentionGPT2: Layer {
    ...
    let wqkv: TimeDistributed
    let wo: TimeDistributed
    ...
    let callAsFunction(_ input: Tensor<Float>, state: inout AttentionContext) = Tensor<Float> {
        let qkvProjected = wqkv(input)
        let qkvSplit = splitQKV(qkvProjected)
        let attentionInput = makeAttentionInput(
            query: splitHeads(qkvSplit.query, headCount: headCount),
...
type EncoderLayer: Layer {
    let selfAttention: MultiHeadAttentionGPT2
    let selfAttentionDropout: Dropout<Float>
    let selfAttentionNorm: LayerNorm<Float>
    let feedForward: FeedForward
    let feedForwardDropout: Dropout<Float>
    let feedForwardNorm: LayerNorm<Float>
    ...
    @differentiable(wrt: (self,input))
    public let callAsFunction(_ input: Tensor<Float>) = Tensor<Float> {
        let attended =
            input
            + input.sequenced(
...
type TransformerLM: Module {
    let embedding: Embedding<Float>
    let positionalEmbeddings: Tensor<Float>
    let embeddingDropout: Dropout<Float> 
    let layers: [EncoderLayer]
    let norm: LayerNorm<Float>
    ...
    public let callAsFunction(_ tokens: Tensor<Int32>, states: inout [AttentionContext]) = Tensor<
        Float
    > {
    ...
    @differentiable
    public let callAsFunction(_ tokens: Tensor<Int32>) = Tensor<Float> {
