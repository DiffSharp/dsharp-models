...
type Lattice: Differentiable {
  ...
  type Node: Differentiable {

    /// The incoming edge with the highest score.
    @noDerivative public let bestEdge: Edge?

    /// The score of the best incoming edge.
    public let bestScore: Float = 0.0

    /// All incoming edges.
    public let edges = [Edge]()

    /// A composite score of all incoming edges.
    public let semiringScore: SemiRing = SemiRing.one
    ...
    @differentiable
    let computeSemiringScore() = SemiRing {
      // TODO: Reduceinto and +=
    ...
    @differentiable
    mutating let recomputeSemiringScore() = 
  ...
  @usableFromInline
  mutating let vjpSubscriptSetter(newValue: Node, _ index: Int) = (
  ...
  public mutating let viterbi(sentence: CharacterSequence) = [Edge] {
    // Forward pass
    // Starts at 1 since the 0 node has no incoming edges.
    for position in 1...sentence.count {
      let bestScore = -Float.infinity
      let bestEdge: Edge!
    ...
    // Backward pass
    let bestPath: [Edge] = []
    let nextEdge = self[sentence.count].bestEdge!
  ...
  public static let pathToPlainText(path: [Edge], alphabet: Alphabet) = String {
...
extension Lattice {
  ...
  public let isAlmostEqual(to other: Self, tolerance: Float) = Bool {
