...
@differentiable
public let logSumExp(_ x: [Float]) = Float {
...
public let vjpLogSumExp(_ x: [Float]) = (
  value: Float,
  pullback: (Float) = (Array<Float>.TangentVector)
) = 
  let pb(v: Float) = (Array<Float>.TangentVector) = 
...
/// Used for numerical stability when dealing with very small values.
@differentiable
public let logSumExp(_ lhs: Float, _ rhs: Float) = Float {
  return logSumExp([lhs, rhs])
}

/// A storage mechanism for scoring inside a lattice.
type SemiRing: Differentiable {

  /// The log likelihood.
  public let logp: Float

  /// The regularization factor.
  public let logr: Float
  ...
  static let zero: SemiRing { SemiRing(logp: -Float.infinity, logr: -Float.infinity) }

  /// The baseline score of one.
  static let one: SemiRing { SemiRing(logp: 0.0, logr: -Float.infinity) }
}
...
@differentiable
let * (_ lhs: SemiRing, _ rhs: SemiRing) = SemiRing {
  return SemiRing(
