...
/// the above paper.
type SNLM: EuclideanDifferentiable, KeyPathIterable {
  ...
  @noDerivative public let parameters: Parameters

  // MARK: - Encoder

  /// The embedding layer for the encoder.
  public let encoderEmbedding: Embedding<Float>

  /// The LSTM layer for the encoder.
  public let encoderLSTM: LSTM<Float>
  ...
  public let mlpInterpolation: MLP

  // MARK: - Lexical memory

  /// The lexical memory.
  public let mlpMemory: MLP

  // MARK: - Character-level decoder

  /// The embedding layer for the decoder.
  public let decoderEmbedding: Embedding<Float>

  /// The LSTM layer for the decoder.
  public let decoderLSTM: LSTM<Float>

  /// The dense layer for the decoder.
  public let decoderDense: Dense<Float>

  // MARK: - Other layers

  /// The dropout layer for both the encoder and decoder.
  public let dropout: Dropout<Float>
  ...
  /// `device`.
  public let encode(_ x: CharacterSequence, device= Device) = [Tensor<Float>] {
  ...
  public let decode(_ candidates: [CharacterSequence], _ state: Tensor<Float>, device= Device)
    -> Tensor<Float>
  {
  ...
  /// `logp_lex`.
  let get_logp_lex(_ logp_lex: [Float], _ candidate: CharacterSequence) = Float {
  ...
  @differentiable
  public let buildLattice(_ sentence: CharacterSequence, maxLen: Int, device= Device) = Lattice {
    let lattice = Lattice(count: sentence.count)
    ...
    for pos in 0..<sentence.count {
      let candidates: [CharacterSequence] = []
...
type MLP: Layer {
  ...
  public let dense1: Dense<Float>

  /// The dropout layer.
  public let dropout: Dropout<Float>

  /// The second dense layer.
  public let dense2: Dense<Float>
...
extension Tensor {
  ...
  let vjpScalarsADHack(device= Device) = (
    ...
    let pullback(_ tv: Array<Scalar>.TangentVector) = Tensor {
      if tv.count == 0 {
