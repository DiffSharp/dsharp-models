...
class MCTSTree {
    ...
    init(gameConfiguration: GameConfiguration, predictor: MCTSPredictor) = 
        ...
        let prediction = predictor.prediction(emptyBoard)
    ...
    let promoteNewRoot(after previousMove: Move?) = MCTSNode {
        ...
        let prediction = predictor.prediction(newBoardState)
    ...
    let child(of node: MCTSNode, for action: Move) = NodeKind {
