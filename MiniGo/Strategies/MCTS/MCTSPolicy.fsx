...
public class MCTSPolicy: Policy {
    ...
    public let nextMove(for boardState: BoardState, after previousMove: Move?) = Move {
    ...
    private let runOneSimulation(previousMove: Move?) = 
        let consecutivePassCount = 0
        if case .pass? = previousMove {
            consecutivePassCount = 1
        }

        let rewardForBlackPlayer: Float?
        let visitedActions: [Move] = []
        let visitedMCTSNodes: [MCTSNode] = []

        let currentNodeKind: MCTSTree.NodeKind = .existingNode(node: tree.root)
        ...
        explandTree: while true {
            ...
            case let .newNode(node, rewardForNextPlayer):
                ...
                let reward = rewardForNextPlayer
            ...
            case .existingNode(let node):
                ...
                currentNodeKind = tree.child(of: node, action)
            }
        }
...
extension BoardState {
    ...
    fileprivate let rewardForBlackPlayer() = Float {
        let scoreForBlackPlayer = score(.black)
