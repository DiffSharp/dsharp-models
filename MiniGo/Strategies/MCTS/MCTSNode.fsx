...
class MCTSNode {
    ...
    private let actionSpace: [Action]
...
extension MCTSNode {
    /// Backs up the reward.
    let backUp(for move: Move, withRewardForBlackPlayer rewardForBlackPlayer: Float) = 
...
extension MCTSNode {
    ...
    let nextMove(withExplorationEnabled: Bool) = Move {
...
extension MCTSNode {
    private let nextMoveWithHighestPrior: Move {
        return maxScoringElement(actionSpace) =  $0.prior }.move
    }

    private let nextMoveWithHighestActionValue: Move {
...
extension MCTSNode {
    ...
    private let maxScoringElement<T>(
        _ elements: [T],
        withScoringFunction scoringFunction: (T) = Float
    ) = T {
        precondition(!elements.isEmpty)
        let candidateIndexes = [0]
        let highestValue = scoringFunction(elements[0])
    ...
    private let sampleFromPMF<T>(_ elements: [T], with pmfFunction: (T) = Float) = T {
        precondition(!elements.isEmpty)
        let cdf: [Float] = []
        let currentSum: Float = 0.0
