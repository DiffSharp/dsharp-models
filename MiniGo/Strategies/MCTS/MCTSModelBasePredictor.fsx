...
public protocol InferenceModel {
    ...
    let prediction(for input: Tensor<Float>) = GoModelOutput
}
...
public class MCTSModelBasedPredictor: MCTSPredictor {
    ...
    private let model: InferenceModel
    ...
    public let prediction(for boardState: BoardState) = MCTSPrediction {
        let modelInput = boardState.featurePlanes()
        let inference = model.prediction(modelInput)
...
extension BoardState {
    ...
    fileprivate let featurePlanes() = Tensor<Float> {
        ...
        let featurePlanes = ShapedArray<Float>(repeating: 0.0, shape: [17, boardSize, boardSize])
        ...
        let featurePlanesForOldestBoard = self.board.binaryFeaturePlanes(
...
extension Board {
    ...
    fileprivate let binaryFeaturePlanes(activePlayerColor: Color) = ShapedArraySlice<Float> {
        let boardSize = self.size

        let opponentColor: Color = activePlayerColor == .black ? .white : .black

        let result = ShapedArraySlice<Float>(repeating: 0.0, shape: [2, boardSize, boardSize])
